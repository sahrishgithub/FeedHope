...
public class RiderRVAdapter extends RecyclerView.Adapter<RiderRVAdapter.UserViewHolder> {
    ...
    public RiderRVAdapter(ArrayList<RiderModalClass> userList, Context context, NotificationManager notifManager) {
        // Filter userList to exclude any null or incomplete entries
        this.userList = new ArrayList<>();
        for (RiderModalClass user : userList) {
    ...
    public void onBindViewHolder(@NonNull UserViewHolder holder, int position) {
        ...
        holder.accept.setOnClickListener(v -> {
            boolean isInserted = db.insertData(user.getName(),user.getPhone(),user.getLicence(),user.getHours(),user.getDays(), user.getCard(), user.getEmail(), user.getPass(), user.getLocation());
    ...
    static class UserViewHolder extends RecyclerView.ViewHolder {
        TextView nameText, phoneText, licenceText, hoursText, daysText, cardText,emailText, passText,locationText;
